...
/*   Updated: 2025/02/07 17:58:29 by zzaoui           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	if (ac == 2)
	{
		...
		if (data.game.mlx == NULL)
			return (EXIT_FAILURE);
		data.game.mlx_win = mlx_new_window(data.game.mlx, 64 * data.map.cols, 64
				* data.map.rows, "so_long");
		...
		mlx_loop_hook(data.game.mlx, render_map, &data);
